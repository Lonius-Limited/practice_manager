...
def cash():
    ...
    result = frappe.db.sql(f"select SUM(amount) as amount from `tabConsult Ledger` where company ='{company}' ", as_dict = 1)
    ...
    result = frappe.db.sql(f"select SUM(paid_amount) as amount from `tabPayment Entry` where company ='{company}' ", as_dict=1)
